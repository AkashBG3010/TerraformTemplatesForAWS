...
resource "aws_iam_role_policy" "my-policy" {
  role = aws_iam_role.my-policy.name
  ...
  policy = <<POLICY
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "${aws_s3_bucket.my-bucket1.arn}",
        "${aws_s3_bucket.my-bucket2.arn}/*"
      ]
    }
  ]
}
...
resource "aws_codebuild_project" "code-build" {
  ...
  build_timeout = "5"
  service_role  = aws_iam_role.my-role.arn
  ...
  logs_config {
    ...
    s3_logs {
      status   = "ENABLED"
      location = "${aws_s3_bucket.my-bucket.id}/build-log"
    }
  }
  ...
  vpc_config {
    ...
    subnets = [
      aws_subnet.my-subnet1.id,
      aws_subnet.my=subnet2.id,
    ]
